...
public class StepDefinitionsForCrossBrowsing {
...
//		options.addArguments("--disable-popup-blocking");
     //  WebDriverManager.chromedriver().setup();
	  //  driver=new ChromeDriver();
		...
		if(GetKey.property.get().getProperty("BrowserType").equalsIgnoreCase("Firefox")) {
		System.setProperty("webdriver.gecko.driver", "C:\\Users\\Dell\\Desktop\\geckodriver.exe");
		driver=new FirefoxDriver();	

		
		}else if(GetKey.property.get().getProperty("BrowserType").equalsIgnoreCase("Chrome")){
			System.setProperty("webdriver.chrome.driver","C:\\Users\\Dell\\Desktop\\chromedriver_win32\\chromedriver.exe");
			driver=new ChromeDriver();
			driver.manage().window().maximize();
		}else if(GetKey.property.get().getProperty("BrowserType").equalsIgnoreCase("Edge")){
